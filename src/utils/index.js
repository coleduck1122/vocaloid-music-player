...
export function formattingTime(msec) {
  let result = ''
...
export function timeDeserialize(timeFormat) {
...
const test = { notSupportedScroll: undefined };
// 格式化歌词字符串
...
// 随机产生指定范围数
export function randomNum(minNum, maxNum, decimals = false){
  if(decimals) {
...
export function formatDate(timestamp, format = 'YY-MM-DD hh:mm:ss') {
...
// 计算出合适的间隙
export function suitableSpace(el, itemEl, count) {
...
// 是否处于今天
export function calculateIsToday(timestamp) {
...
// 解析路径参数
export function parsePathQuery(path) {
  const result = {
    path: path,
    query: {},
  }
...
export function animation(time, cb) {
  let start;
  let intervalId;
  let elapsed = 0;
  let paused = false;
...
export function isGoodColor(r, g, b) {
...
export function findBestColors(colors, num) {
